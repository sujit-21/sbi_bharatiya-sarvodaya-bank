...
function showDashboard() {
  ...
  if (branchIndicator) {
    ...
    const fullName = state.user?.fullName || userName || 'Staff User';
    const roleLower = (state.user?.role || '').toLowerCase();
    const roleLabel = roleLower.includes('manager') ? 'Manager' : 'Employee';

    branchIndicator.innerHTML = `
      <div style="font-weight: 800; font-size: 0.82rem; color: #047857; line-height: 1.25;">Branch : ${branchName}</div>
      <div style="font-weight: 600; font-size: 0.76rem; color: #065f46; margin-top: 2px; line-height: 1.25;">${roleLabel} : ${fullName}</div>
    `;
  }
...
async function renderDepositWithdrawWorkspace(container) {
  try {
    ...
    const [custRes, summaryRes] = await Promise.all([
      apiCall('/api/branches/' + encodeURIComponent(activeBranchId) + '/customers', 'GET', null, true)
...
window.selectCounterCustomer = function(custId) {
  const cust = (window._counterCustomersList || []).find(c => (c.id === custId || c.userId === custId));
  if (!cust) return;
  
  window._counterSelectedCustomer = cust;
  window._counterSelectedAccount = (cust.accounts && cust.accounts[0]) || {
    accountNumber: cust.accountNumber || 'ACC-1001',
    balance: cust.totalBalance || cust.balance || 0,
    type: 'SAVINGS'
  };
  ...
  // Update Snapshot Card Fields
  const nameEl = document.getElementById('snap-cust-name');
  if (nameEl) nameEl.innerText = selCust.fullName || 'Customer';
  const idEl = document.getElementById('snap-cust-id');
  if (idEl) idEl.innerText = selCust.userId || selCust.id || '-';
  const mobEl = document.getElementById('snap-cust-mobile');
  if (mobEl) mobEl.innerText = selCust.mobileNumber || selCust.phone || '9876543210';
  const balEl = document.getElementById('snap-acc-balance');
  if (balEl) balEl.innerText = `₹${parseFloat(selAcc.balance || selCust.totalBalance || selCust.balance || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`;
  const noEl = document.getElementById('snap-acc-no');
  if (noEl) noEl.innerText = selAcc.accountNumber || selCust.accountNumber || 'ACC-1001';
  const typeEl = document.getElementById('snap-acc-type');
  if (typeEl) typeEl.innerText = `${(selAcc.type || 'SAVINGS').toUpperCase()} ACCOUNT`;
  const mopEl = document.getElementById('snap-acc-mop');
  if (mopEl) mopEl.innerText = selAcc.mopType || selCust.mopType || 'Self';

  // Update Form target/source fields
  const depAcc = document.getElementById('dep-target-account');
  if (depAcc) depAcc.value = selAcc.accountNumber || selCust.accountNumber || 'ACC-1001';
  const wthAcc = document.getElementById('wth-source-account');
  if (wthAcc) wthAcc.value = selAcc.accountNumber || selCust.accountNumber || 'ACC-1001';

  checkWithdrawalSufficiency();
};

window.filterCounterCustomerDropdown = function(query) {
  const q = (query || '').toLowerCase().trim();
  const select = document.getElementById('counter-customer-select');
  if (!select) return;
  ...
  list.forEach((c, idx) => {
    const accNo = (c.accounts && c.accounts[0] && c.accounts[0].accountNumber) || c.accountNumber || 'ACC-1001';
    const text = `${c.fullName || ''} ${accNo} ${c.userId || ''} ${c.mobileNumber || ''} ${c.panNumber || ''}`.toLowerCase();
    if (!q || text.includes(q)) {
      const opt = document.createElement('option');
      opt.value = c.id || c.userId;
      opt.style = 'padding: 6px 8px; border-bottom: 1px solid #f1f5f9;';
      opt.innerText = `${c.fullName || 'Customer'} — ${accNo} (₹${(parseFloat(c.totalBalance || c.balance || 0)).toLocaleString('en-IN')})`;
  ...
  if (select.options.length > 0) {
    ...
    selectCounterCustomer(select.options[0].value);
  }
};
