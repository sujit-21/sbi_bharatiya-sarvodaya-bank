...
function checkRememberedUser() {
  try {
    const isRemembered = localStorage.getItem('hq_remember_me') === 'true';
    const rememberedEmail = localStorage.getItem('hq_remember_email');
  ...
  try {
    ...
    try {
      ...
      if (rememberEl && rememberEl.checked) {
        localStorage.setItem('hq_remember_email', email);
        localStorage.setItem('hq_remember_me', 'true');
      } else {
        localStorage.removeItem('hq_remember_email');
        localStorage.removeItem('hq_remember_me');
      }
    } catch(e) {}
...
window.selectDemoAccount = function(email, password) {
  ...
  document.querySelectorAll('.quick-pill').forEach(btn => {
    ...
    } else if (btn.innerText.includes('Auditor') && email.includes('auditor')) {
      btn.style.borderColor = '#bfdbfe';
      btn.style.background = '#eff6ff';
      btn.style.color = '#1d4ed8';
      btn.style.fontWeight = '700';
...
window.switchRecoveryTab = function(tab) {
  ...
  if (tab === 'userid') {
    ...
    if (title) title.innerText = 'Retrieve HQ User ID';
...
window.handleForgotUserId = function(e) {
  ...
  let roleName = 'Super Admin HQ';
  if (email.includes('auditor')) {
    recoveredId = 'auditor@bank.com';
    roleName = 'System Auditor HQ';
  } else if (email.includes('compliance')) {
    recoveredId = 'compliance@bank.com';
    roleName = 'HQ Compliance Officer';
  }

  resultDiv.innerHTML = `
    <div style="font-weight: 700; margin-bottom: 4px;">✅ Verified HQ Account Found</div>
